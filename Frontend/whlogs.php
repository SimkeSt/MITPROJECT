...
<div class="button-container" style="margin-top: 20%; margin-right: 25%; margin-left: 25%;">
    <h2 class="text-light">Import Record for <?php 
    $dik=$_POST['namaj'];
    echo "$dik"; ?></h2>
    <table class="table table-success" style="margin-top:0%;">
      ...
      <th scope="col">Price</th>
                        ...
                        if (isset($_SESSION['username'])){
                            $whid=$_POST['id'];
                            $sql = "SELECT * FROM ImportRecord WHERE WHID= $whid";
                            $result = $db->query($sql);
                            if (isset($_SESSION['username'])){
                                if ($result->num_rows > 0) {
                                      while($row = $result->fetch_assoc()) {
                                        $pi=$row['PriceImportedAt'];
                                        $qt=$row["IQuantity"];
                                        $idd=$row["ImportDate"];
                                        echo "<td>$idd</td>
                                        <td>$qt</td>
                        ...
                        <br><br>
                        <h2 class="text-light">Export Record for <?php 
    $dik=$_POST['namaj'];
    echo "$dik"; ?></h2>
    <table class="table table-success" style="margin-top:0%;">
         <thead>
      ...
      <th scope="col">Price</th>
                        ...
                        if (isset($_SESSION['username'])){
                            $whid=$_POST['id'];
                            $sql = "SELECT * FROM SaleRecord WHERE WHID= $whid";
                            $result = $db->query($sql);
                            if (isset($_SESSION['username'])){
                                if ($result->num_rows > 0) {
                                      while($row = $result->fetch_assoc()) {
