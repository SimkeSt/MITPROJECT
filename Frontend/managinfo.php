...
<div class="button-container" style="margin-top: 15%; margin-right: 25%; margin-left: 25%;">
                        <?php
                        if (isset($_SESSION['username'])){
                        echo '<form action="mang.php" method="get" style="display: flex;">
                        <select class="form-select" aria-label="Default select example" name="selected" id="selected">
                        <option selected>Open this select menu</option>';
                          ...
                          $result = $db->query($sql);
                                if ($result->num_rows > 0) {
                                      while($row = $result->fetch_assoc()) {
                                        $id=$row['WHID'];
                                        $name="Warehouse Name: " . $row["WName"] ."| Adress: " . $row["Adress"]." |";
                                        echo "<option value='$id'>$name</option>";}
                                    }
                        echo '</select>
                        &nbsp; &nbsp;<input type="submit" name="select" class="btn btn-success" value=" See manager">
                        </form>';
                        }
                        ?>
                    </div> 
